export const DB = {
    techs: [
        { id: "WEAP", name: "Weapons", desc: "Offensive systems and targeting.", baseCost: 500 },
        { id: "PROP", name: "Propulsion", desc: "Engines, fuel, and range.", baseCost: 450 },
        { id: "CONST", name: "Construction", desc: "Hull materials and industry.", baseCost: 420 },
        { id: "ELEC", name: "Electronics", desc: "Sensors, automation, and computers.", baseCost: 380 },
        { id: "ENER", name: "Energy", desc: "Reactors, shields, and power systems.", baseCost: 400 },
        { id: "BIOT", name: "Biotechnology", desc: "Life support and terraforming.", baseCost: 460 },
        { id: "TERR", name: "Terraforming", desc: "Planetary engineering.", baseCost: 440 }
    ],
    hulls: [
        {
            id: "scout",
            name: "Scout",
            baseMass: 40,
            maxMass: 90,
            armor: 25,
            structure: 60,
            baseFuel: 70,
            baseCargo: 10,
            baseInitiative: 6,
            baseSpeed: 6,
            baseRange: 120,
            speedMassFactor: 140,
            fuelRangeFactor: 12,
            signature: 4,
            scanner: 120,
            camo: 12,
            cost: 50,
            tech: { CONST: 1 },
            slotLayout: { engine: 1, weapon: 1, utility: 1 }
        },
        {
            id: "frigate",
            name: "Frigate",
            baseMass: 80,
            maxMass: 160,
            armor: 60,
            structure: 110,
            baseFuel: 110,
            baseCargo: 20,
            baseInitiative: 5,
            baseSpeed: 5,
            baseRange: 130,
            speedMassFactor: 150,
            fuelRangeFactor: 12,
            signature: 6,
            scanner: 130,
            camo: 10,
            cost: 140,
            tech: { CONST: 1 },
            slotLayout: { engine: 1, weapon: 1, utility: 2 }
        },
        {
            id: "destroyer",
            name: "Destroyer",
            baseMass: 140,
            maxMass: 260,
            armor: 110,
            structure: 200,
            baseFuel: 160,
            baseCargo: 30,
            baseInitiative: 4,
            baseSpeed: 4,
            baseRange: 140,
            speedMassFactor: 165,
            fuelRangeFactor: 14,
            signature: 8,
            scanner: 140,
            camo: 8,
            cost: 280,
            tech: { CONST: 2 },
            slotLayout: { engine: 1, weapon: 2, utility: 2 }
        },
        {
            id: "cruiser",
            name: "Cruiser",
            baseMass: 220,
            maxMass: 400,
            armor: 180,
            structure: 320,
            baseFuel: 220,
            baseCargo: 45,
            baseInitiative: 3,
            baseSpeed: 3,
            baseRange: 150,
            speedMassFactor: 180,
            fuelRangeFactor: 15,
            signature: 10,
            scanner: 150,
            camo: 6,
            cost: 520,
            tech: { CONST: 3 },
            slotLayout: { engine: 1, weapon: 3, utility: 2 }
        },
        {
            id: "battleship",
            name: "Battleship",
            baseMass: 360,
            maxMass: 680,
            armor: 320,
            structure: 520,
            baseFuel: 300,
            baseCargo: 70,
            baseInitiative: 2,
            baseSpeed: 2,
            baseRange: 160,
            speedMassFactor: 200,
            fuelRangeFactor: 16,
            signature: 14,
            scanner: 160,
            camo: 4,
            cost: 980,
            tech: { CONST: 4 },
            slotLayout: { engine: 1, weapon: 4, utility: 3 }
        },
        {
            id: "small_freighter",
            name: "Small Freighter",
            baseMass: 25,
            maxMass: 80,
            armor: 15,
            structure: 40,
            baseFuel: 130,
            baseCargo: 120,
            baseInitiative: 1,
            baseSpeed: 4,
            baseRange: 140,
            speedMassFactor: 140,
            fuelRangeFactor: 14,
            signature: 6,
            scanner: 80,
            camo: 2,
            cost: 60,
            tech: { CONST: 0 },
            slotLayout: { engine: 1, utility: 1 }
        },
        {
            id: "colony_ship",
            name: "Colony Ship",
            baseMass: 20,
            maxMass: 90,
            armor: 20,
            structure: 50,
            baseFuel: 200,
            baseCargo: 30,
            baseInitiative: 2,
            baseSpeed: 4,
            baseRange: 150,
            speedMassFactor: 150,
            fuelRangeFactor: 14,
            signature: 6,
            scanner: 90,
            camo: 4,
            cost: 80,
            tech: { CONST: 0 },
            slotLayout: { engine: 1, utility: 1 }
        },
        {
            id: "bomber",
            name: "Bomber",
            baseMass: 28,
            maxMass: 110,
            armor: 25,
            structure: 65,
            baseFuel: 120,
            baseCargo: 10,
            baseInitiative: 3,
            baseSpeed: 5,
            baseRange: 140,
            speedMassFactor: 145,
            fuelRangeFactor: 12,
            signature: 7,
            scanner: 100,
            camo: 4,
            cost: 90,
            tech: { CONST: 1 },
            slotLayout: { engine: 1, weapon: 1, utility: 1 }
        },
        {
            id: "galleon",
            name: "Galleon",
            baseMass: 125,
            maxMass: 320,
            armor: 120,
            structure: 200,
            baseFuel: 2500,
            baseCargo: 1000,
            baseInitiative: 1,
            baseSpeed: 2,
            baseRange: 200,
            speedMassFactor: 220,
            fuelRangeFactor: 20,
            signature: 18,
            scanner: 120,
            camo: 2,
            cost: 400,
            tech: { CONST: 3 },
            slotLayout: { engine: 1, utility: 6 }
        },
        {
            id: "meta_morph",
            name: "Meta Morph",
            baseMass: 60,
            maxMass: 130,
            armor: 40,
            structure: 90,
            baseFuel: 90,
            baseCargo: 15,
            baseInitiative: 5,
            baseSpeed: 5,
            baseRange: 130,
            speedMassFactor: 145,
            fuelRangeFactor: 12,
            signature: 5,
            scanner: 130,
            camo: 6,
            cost: 120,
            tech: { CONST: 1 },
            slotLayout: { engine: 1, weapon: 1, utility: 2 },
            reqTrait: "HE",
            requiresTraits: ["HE"]
        },
        {
            id: "mini_colonizer",
            name: "Mini Colonizer",
            baseMass: 18,
            maxMass: 70,
            armor: 18,
            structure: 45,
            baseFuel: 180,
            baseCargo: 25,
            baseInitiative: 2,
            baseSpeed: 4,
            baseRange: 150,
            speedMassFactor: 150,
            fuelRangeFactor: 14,
            signature: 5,
            scanner: 90,
            camo: 4,
            cost: 60,
            tech: { CONST: 0, BIOT: 1 },
            slotLayout: { engine: 1, utility: 1 },
            reqTrait: "HE",
            requiresTraits: ["HE"]
        },
        {
            id: "rogue",
            name: "Rogue",
            baseMass: 55,
            maxMass: 120,
            armor: 35,
            structure: 80,
            baseFuel: 100,
            baseCargo: 10,
            baseInitiative: 5,
            baseSpeed: 6,
            baseRange: 150,
            speedMassFactor: 140,
            fuelRangeFactor: 12,
            signature: 3,
            scanner: 140,
            camo: 18,
            cost: 150,
            tech: { CONST: 2, ELEC: 3 },
            slotLayout: { engine: 1, weapon: 2, utility: 1 },
            reqTrait: "SS",
            requiresTraits: ["SS"]
        },
        {
            id: "stealth_bomber",
            name: "Stealth Bomber",
            baseMass: 32,
            maxMass: 120,
            armor: 30,
            structure: 70,
            baseFuel: 120,
            baseCargo: 10,
            baseInitiative: 4,
            baseSpeed: 5,
            baseRange: 150,
            speedMassFactor: 145,
            fuelRangeFactor: 12,
            signature: 4,
            scanner: 110,
            camo: 20,
            cost: 120,
            tech: { CONST: 2, WEAP: 2 },
            slotLayout: { engine: 1, weapon: 2, utility: 1 },
            reqTrait: "SS",
            requiresTraits: ["SS"]
        },
        {
            id: "battle_cruiser",
            name: "Battle Cruiser",
            baseMass: 300,
            maxMass: 520,
            armor: 260,
            structure: 420,
            baseFuel: 280,
            baseCargo: 60,
            baseInitiative: 2,
            baseSpeed: 3,
            baseRange: 155,
            speedMassFactor: 190,
            fuelRangeFactor: 16,
            signature: 12,
            scanner: 160,
            camo: 4,
            cost: 780,
            tech: { CONST: 4 },
            slotLayout: { engine: 1, weapon: 4, utility: 3 },
            reqTrait: "WM",
            requiresTraits: ["WM"]
        },
        {
            id: "dreadnought",
            name: "Dreadnought",
            baseMass: 500,
            maxMass: 900,
            armor: 420,
            structure: 700,
            baseFuel: 340,
            baseCargo: 90,
            baseInitiative: 1,
            baseSpeed: 2,
            baseRange: 170,
            speedMassFactor: 220,
            fuelRangeFactor: 17,
            signature: 16,
            scanner: 170,
            camo: 3,
            cost: 1400,
            tech: { CONST: 5 },
            slotLayout: { engine: 1, weapon: 5, utility: 4 },
            reqTrait: "WM",
            requiresTraits: ["WM"]
        },
        {
            id: "mini_mine_layer",
            name: "Mini Mine Layer",
            baseMass: 30,
            maxMass: 90,
            armor: 25,
            structure: 60,
            baseFuel: 120,
            baseCargo: 10,
            baseInitiative: 3,
            baseSpeed: 5,
            baseRange: 140,
            speedMassFactor: 145,
            fuelRangeFactor: 12,
            signature: 6,
            scanner: 100,
            camo: 4,
            cost: 80,
            tech: { CONST: 1 },
            slotLayout: { engine: 1, utility: 2 },
            reqTrait: "SD",
            requiresTraits: ["SD"]
        },
        {
            id: "super_mine_layer",
            name: "Super Mine Layer",
            baseMass: 200,
            maxMass: 380,
            armor: 160,
            structure: 280,
            baseFuel: 200,
            baseCargo: 20,
            baseInitiative: 2,
            baseSpeed: 3,
            baseRange: 150,
            speedMassFactor: 175,
            fuelRangeFactor: 15,
            signature: 10,
            scanner: 140,
            camo: 4,
            cost: 480,
            tech: { CONST: 3 },
            slotLayout: { engine: 1, utility: 4 },
            reqTrait: "SD",
            requiresTraits: ["SD"]
        },
        {
            id: "starbase",
            name: "Starbase",
            type: "starbase",
            baseMass: 1200,
            maxMass: 2000,
            armor: 500,
            structure: 800,
            baseFuel: 500,
            baseCargo: 0,
            baseInitiative: 0,
            baseSpeed: 0,
            baseRange: 0,
            speedMassFactor: 9999,
            fuelRangeFactor: 1,
            signature: 20,
            scanner: 200,
            camo: 0,
            cost: 1200,
            tech: { CONST: 0 },
            slotLayout: { weapon: 12, utility: 8 }
        }
    ],
    components: {
        engines: [
            {
                id: "ion_drive",
                name: "Ion Drive",
                slotType: "engine",
                mass: 18,
                cost: 40,
                powerOutput: 8,
                powerUsage: 0,
                stats: {
                    speed: 9,
                    range: 180,
                    fuel: 20,
                    fuelUsage: [0.6, 0.8, 1.1, 1.5, 2.0, 2.6, 3.3, 4.1, 5.0, 6.0]
                },
                tech: { PROP: 1 }
            },
            {
                id: "ramscoop_drive",
                name: "Ramscoop Drive",
                slotType: "engine",
                mass: 36,
                cost: 200,
                powerOutput: 16,
                powerUsage: 0,
                flags: ["ramscoop"],
                stats: {
                    speed: 10,
                    range: 220,
                    fuel: 30,
                    freeSpeed: 6,
                    fuelUsage: [0.3, 0.45, 0.6, 0.8, 1.1, 1.4, 1.8, 2.3, 2.9, 3.6]
                },
                tech: { PROP: 5 }
            },
            {
                id: "fusion_burn",
                name: "Fusion Burner",
                slotType: "engine",
                mass: 32,
                cost: 120,
                powerOutput: 14,
                powerUsage: 0,
                stats: {
                    speed: 14,
                    range: 240,
                    fuel: 40,
                    initiative: 1,
                    fuelUsage: [0.5, 0.7, 1.0, 1.3, 1.7, 2.2, 2.8, 3.5, 4.3, 5.2]
                },
                tech: { PROP: 3 }
            },
            {
                id: "quantum_surge",
                name: "Quantum Surge",
                slotType: "engine",
                mass: 46,
                cost: 240,
                powerOutput: 20,
                powerUsage: 0,
                stats: {
                    speed: 20,
                    range: 320,
                    fuel: 60,
                    initiative: 2,
                    fuelUsage: [0.45, 0.6, 0.85, 1.2, 1.6, 2.1, 2.7, 3.4, 4.2, 5.1]
                },
                tech: { PROP: 5 }
            },
            {
                id: "warp_shear",
                name: "Warp Shear",
                slotType: "engine",
                mass: 58,
                cost: 360,
                powerOutput: 26,
                powerUsage: 0,
                stats: {
                    speed: 26,
                    range: 400,
                    fuel: 90,
                    initiative: 3,
                    fuelUsage: [0.4, 0.55, 0.8, 1.1, 1.5, 2.0, 2.6, 3.3, 4.1, 5.0]
                },
                tech: { PROP: 7 }
            }
        ],
        weapons: [
            {
                id: "laser_array",
                name: "X-Ray Laser",
                slotType: "weapon",
                mass: 12,
                cost: 30,
                powerUsage: 4,
                stats: { beamDamage: 18, beamRange: 1, gattling: 1 },
                tech: { WEAP: 1 }
            },
            {
                id: "mass_driver",
                name: "Mass Driver",
                slotType: "weapon",
                mass: 18,
                cost: 55,
                powerUsage: 5,
                stats: { torpedoDamage: 28, torpedoRange: 2, gattling: 2, bombing: 3 },
                tech: { WEAP: 2 }
            },
            {
                id: "plasma_lance",
                name: "Plasma Lance",
                slotType: "weapon",
                mass: 24,
                cost: 90,
                powerUsage: 7,
                stats: { beamDamage: 40, beamRange: 1, sapper: 0.1 },
                tech: { WEAP: 3 }
            },
            {
                id: "nova_driver",
                name: "Nova Driver",
                slotType: "weapon",
                mass: 36,
                cost: 160,
                powerUsage: 10,
                stats: { torpedoDamage: 75, torpedoRange: 3, bombing: 8 },
                tech: { WEAP: 5 }
            },
            {
                id: "singularity_cannon",
                name: "Singularity Cannon",
                slotType: "weapon",
                mass: 48,
                cost: 260,
                powerUsage: 14,
                stats: { beamDamage: 110, beamRange: 2, sapper: 0.25, initiative: -1 },
                tech: { WEAP: 7 }
            },
            {
                id: "retro_bomb",
                name: "Retro Bomb",
                slotType: "weapon",
                mass: 20,
                cost: 90,
                powerUsage: 4,
                stats: { torpedoDamage: 40, torpedoRange: 1, bombing: 12 },
                tech: { WEAP: 4 },
                reqTrait: "WM",
                requiresTraits: ["WM"]
            }
        ],
        utilities: [
            {
                id: "shield_array",
                name: "Shield Array",
                slotType: "utility",
                mass: 14,
                cost: 60,
                powerUsage: 6,
                stats: { defense: 12, shields: 18 },
                tech: { ENER: 2 }
            },
            {
                id: "deflector_screen",
                name: "Deflector Screen",
                slotType: "utility",
                mass: 20,
                cost: 110,
                powerUsage: 8,
                stats: { defense: 20, shields: 30 },
                tech: { ENER: 4 }
            },
            {
                id: "scanner_array",
                name: "Scanner Array",
                slotType: "utility",
                mass: 10,
                cost: 40,
                powerUsage: 3,
                stats: { range: 80, scanner: 80, initiative: 1 },
                tech: { ELEC: 1 }
            },
            {
                id: "battle_computer",
                name: "Battle Computer",
                slotType: "utility",
                mass: 12,
                cost: 70,
                powerUsage: 4,
                stats: { targeting: 8, initiative: 1 },
                tech: { ELEC: 3 }
            },
            {
                id: "reactor_core",
                name: "Reactor Core",
                slotType: "utility",
                mass: 16,
                cost: 70,
                powerOutput: 12,
                powerUsage: 0,
                stats: {},
                tech: { ENER: 1 }
            },
            {
                id: "colony_pod",
                name: "Colony Pod",
                slotType: "utility",
                mass: 24,
                cost: 120,
                powerUsage: 2,
                stats: { cargo: 60 },
                flags: ["colonize"],
                tech: { BIOT: 1 }
            },
            {
                id: "minelayer_rig",
                name: "Minelayer Rig",
                slotType: "utility",
                mass: 22,
                cost: 110,
                powerUsage: 4,
                stats: { mineCapacity: 80 },
                flags: ["minelayer"],
                tech: { WEAP: 3 }
            },
            {
                id: "armor_plating",
                name: "Armor Plating",
                slotType: "utility",
                mass: 20,
                cost: 55,
                powerUsage: 0,
                stats: { armor: 30, structure: 10, defense: 4 },
                tech: { CONST: 1 }
            },
            {
                id: "reinforced_bulkhead",
                name: "Reinforced Bulkhead",
                slotType: "utility",
                mass: 28,
                cost: 90,
                powerUsage: 0,
                stats: { armor: 45, structure: 20, defense: 6 },
                tech: { CONST: 4 }
            },
            {
                id: "cargo_bay",
                name: "Cargo Bay",
                slotType: "utility",
                mass: 16,
                cost: 35,
                powerUsage: 0,
                stats: { cargo: 40 },
                tech: { CONST: 1 }
            },
            {
                id: "fuel_pod",
                name: "Fuel Pod",
                slotType: "utility",
                mass: 18,
                cost: 45,
                powerUsage: 0,
                stats: { fuel: 60 },
                tech: { PROP: 2 }
            },
            {
                id: "psionic_shield",
                name: "Psionic Shield",
                slotType: "utility",
                mass: 18,
                cost: 140,
                powerUsage: 6,
                stats: { defense: 18, shields: 26, initiative: 2, camo: 12 },
                tech: { ENER: 5, ELEC: 4 },
                requiresTraits: ["psionic_innovators"]
            },
            {
                id: "settlers_delight",
                name: "Settler's Delight",
                slotType: "utility",
                mass: 26,
                cost: 140,
                powerUsage: 2,
                stats: { cargo: 80, range: 20 },
                tech: { BIOT: 2 },
                reqTrait: "HE",
                requiresTraits: ["HE"]
            },
            {
                id: "flux_capacitor",
                name: "Flux Capacitor",
                slotType: "utility",
                mass: 20,
                cost: 160,
                powerOutput: 20,
                powerUsage: 0,
                stats: { initiative: 1 },
                tech: { ENER: 4 },
                reqTrait: "PP",
                requiresTraits: ["PP"]
            },
            {
                id: "pick_pocket_scanner",
                name: "Pick Pocket Scanner",
                slotType: "utility",
                mass: 12,
                cost: 120,
                powerUsage: 4,
                stats: { scanner: 120, camo: 8 },
                tech: { ELEC: 4 },
                reqTrait: "SS",
                requiresTraits: ["SS"]
            },
            {
                id: "chameleon_scanner",
                name: "Chameleon Scanner",
                slotType: "utility",
                mass: 16,
                cost: 180,
                powerUsage: 5,
                stats: { scanner: 160, camo: 14 },
                tech: { ELEC: 6 },
                reqTrait: "SS",
                requiresTraits: ["SS"]
            },
            {
                id: "shadow_shield",
                name: "Shadow Shield",
                slotType: "utility",
                mass: 22,
                cost: 200,
                powerUsage: 6,
                stats: { defense: 16, shields: 24, camo: 12 },
                tech: { ENER: 4 },
                reqTrait: "SS",
                requiresTraits: ["SS"]
            },
            {
                id: "speed_trap_mines",
                name: "Speed Trap Mines",
                slotType: "utility",
                mass: 24,
                cost: 130,
                powerUsage: 4,
                stats: { mineCapacity: 120 },
                flags: ["minelayer", "speed_trap"],
                tech: { WEAP: 4 },
                reqTrait: "SD",
                requiresTraits: ["SD"]
            }
        ]
    },
    structures: {
        mine: { name: "Mine", cost: 6 },
        factory: { name: "Factory", cost: 8 },
        base: { name: "Starbase", cost: 600 }
    }
};
